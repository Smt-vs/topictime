...
Applicativo Next.js per TopicTime: landing pubblica, login, stanze tematiche a tempo generate con topic casuali, chatroom complete, utenti in stanza, wallet con valuta Star, regalo gratuito giornaliero, temi acquistabili, Premium, profilo per interessi, match post-room, spazio community, notifiche e moderazione.
...
- Landing collegata al flusso reale: `/` mostra il racconto pubblico, `/applicativo?auth=register` apre la registrazione, `/applicativo?auth=login` apre l'accesso e gli inviti stanza usano `/applicativo?room=slug`.
...
- Spazio community: feedback aperti, voti, roadmap trasparente e reward +5 Star per proposta salvata.
...
Su Vercel usa `applicativo` come Root Directory del progetto.

La struttura pubblica e:

- `/` mostra la landing.
- `/applicativo` apre l'app con login, stanze, wallet e chatroom.
- `/applicativo/wallet`, `/applicativo/community`, `/applicativo/profile`, `/applicativo/radar` e `/applicativo/support` sono le pagine interne dell'app.
- Le vecchie route `/rooms`, `/wallet`, `/community`, `/profile`, `/radar` e `/support` reindirizzano ai nuovi percorsi sotto `/applicativo`.
- Le vecchie URL statiche della landing (`/index.html`, `/funziona.html`, `/community.html`, ecc.) vengono reindirizzate alle nuove route corrette.
...
Aggiungi anche gli stessi domini in `Redirect URLs`, includendo il ritorno alla app per verifica email e recupero password: `http://localhost:3000/**`, `http://localhost:3000/applicativo`, `http://localhost:3000/auth/confirm`, l'URL production Vercel con `/**`, `/applicativo` e `/auth/confirm`, e se usi preview il pattern Vercel del team. In `Authentication > Providers > Email` abilita Email/Password e lascia attiva la conferma email per verificare i nuovi account.

Se personalizzi il template della mail di conferma, usa un link verso `/auth/confirm` con `token_hash`, `type` e `next`, ad esempio:

```html
{{ .SiteURL }}/auth/confirm?token_hash={{ .TokenHash }}&type=email&next=/applicativo
```
